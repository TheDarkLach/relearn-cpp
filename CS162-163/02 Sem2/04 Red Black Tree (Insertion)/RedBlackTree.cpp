...
int main(){
	...
	"2.Through a text file (Comma Separated w/no Space)\n>>";
	int in;
	cin >> in;
	...
	if (in == 1){
		char input[100];
		cin.ignore(10000,'\n');
		cout << "Enter in your Numbers Separated by a Comma w/no Space:\n>>";
		cin.get(input,1000);
		cin.clear();
		cin.ignore(10000,'\n');
		...
		if (!myfile){
			...
			for(int i = 0; i < integers.size();i++){
				cout << "Inserting the Integer: " << integers[i] << endl;
				Node* newNode = new Node();
				newNode->setValue(integers[i]);
				Root = insert(Root, newNode);
			}
...
void parseNumbers(char input[], vector<int> &integers){
	...
	int exponent = 0;
	//cout << "spacePointers[0] is: " << spacePointers[0] << endl << "spacePointers[1] is: " << spacePointers[1] << endl;
	for (int k = (int)strlen(input)-2; k > spacePointers[0]; k--){
		//cout << "input[k] is: " << input[k] << endl;
		//cout << "input[k] - 48: " << (int)(input[k] - 48) << endl;
		//cout << "Exponent: " << pow(10,exponent) << endl;
		number += (input[k] - 48) * pow(10,exponent);
		exponent++;
